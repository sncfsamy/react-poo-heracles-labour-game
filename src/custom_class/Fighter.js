...
class Fighter {
    ...
    looseWeapon(fighter) {
        ...
        switch(Math.ceil(Math.random() * 10)) {
            case 1:
                msg = <div><b className="loosing">{this.emoji + " " + this.name} a perdu son </b><b className="weapon">{this.weapon.name}</b><b className="loosing"> !</b></div>;
                break;
            case 2:
                msg =  <div><b className="loosing">{this.emoji + " " + this.name} a perdu son </b><b className="weapon">{this.weapon.name}</b><b className="loosing"> à cause de {fighter.emoji + " " + fighter.name} !</b></div>;
                break;
            case 3:
                msg =  <div><b className="loosing">{fighter.emoji + " " + fighter.name} a cassé </b><b className="weapon">l'{this.weapon.name}</b><b className="loosing"> de {this.emoji + " " + this.name}</b></div>;
                break;
            case 4:
                msg =  <div><b className="loosing">{fighter.emoji + " " + fighter.name} a avalé </b><b className="weapon">l'{this.weapon.name}</b><b className="loosing"> de {this.emoji + " " + this.name}</b></div>;
                break;
            case 5:
                msg =  <div><b className="loosing">{fighter.emoji + " " + fighter.name} a gardé </b><b className="weapon">l'{this.weapon.name}</b><b className="loosing"> de {this.emoji + " " + this.name} enfoncée dans son flanc !</b></div>;
                break;
            case 6:
                msg =  <div><b className="loosing">{this.emoji + " " + this.name} a laissé tomber son </b><b className="weapon">{this.weapon.name}</b><b className="loosing"> !</b></div>;
                break;
            default:
                msg =  <div><b className="loosing">{this.emoji + " " + this.name} a laissé échaper son </b><b className="weapon">{this.weapon.name}</b><b className="loosing"> !</b></div>;
        }
        this.weapon = null;
        return msg;
    }
    ...
    looseShield() {
        const msg = <div><b className="loosing">{this.emoji + " " + this.name} a perdu son </b><b className="shield">{this.shield.name}</b><b className="loosing"> !</b></div>;
        this.shield = null;
        return msg;
    }
    ...
    getName() {
        return (this.shield?"🛡️":"") + (this.weapon? "🗡️" : "") + " " + this.emoji + " " + this.name;
    }

    getReverseName() {
        return this.emoji + " " + this.name + " " + (this.weapon? "🗡️" : "") + (this.shield?"🛡️":"");
    }
    ...
    getDamage(fighter) {
        ...
        if (attack <= 1)
            if ((Math.random() + Math.random() + Math.random() +Math.random() + this.dexterity/25 + this.badChance) > 3.2 || (this.canHaveWeapon && Math.random()>0.9) || (this.needWeapon && Math.random()>0.7 && Math.random()>0.7 && Math.random()>0.7)) {
             ...
             msg = <div><span className="chance">{this.emoji + " " + this.name} réussi à attaquer par chance ! (<b> {attack.toFixed(2)}pts de dégats</b> dont <b>{this.badChance.toFixed(2)}pts de dégats de chance</b> !)</span></div>;
             this.badChance = 0;
    ...
    fight(fighter) {
        ...
        return <>{chance}{looseWeapon?looseWeapon:<></>}{looseShield?looseShield:<></>}{fighter.life > 0 ? 
            <div>{this.getName()} ⚔️ {fighter.emoji + " " + fighter.name}: {fighter.getLife()}</div> : 
            <div><br /><br />🏆 {this.emoji + " " + this.name} a gagné ! ({this.getLife()})<br />💀 {fighter.emoji + " " + fighter.name} est mort</div>}
        </>;
    }

    getScore() {
        return <><div>{this.emoji + " " + this.name}</div><div><span id={"win" + this.id}>&nbsp;{this.winCount}</span></div><div><span id={"death" + this.id}>&nbsp;{this.deathCount}</span></div></>;
    }
    ...
    heal(healthpoints) {
        ...
        return <div>{this.emoji + " " + this.name} a été soigné de {Math.floor(healthpoints)} 💙</div>;
    }
}
